...
func (uq *UserQuery) QueryWastes() *WasteQuery {
	query := &WasteQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withWastes: uq.withWastes.Clone(),
...
func (uq *UserQuery) WithWastes(opts ...func(*WasteQuery)) *UserQuery {
	query := &WasteQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withWastes; query != nil {
		if err := uq.loadWastes(ctx, query, nodes,
			func(n *User) { n.Edges.Wastes = []*Waste{} },
			func(n *User, e *Waste) { n.Edges.Wastes = append(n.Edges.Wastes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadWastes(ctx context.Context, query *WasteQuery, nodes []*User, init func(*User), assign func(*User, *Waste)) error {
	...
	query.Where(predicate.Waste(func(s *sql.Selector) {
		s.Where(sql.InValues(user.WastesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_wastes
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_wastes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_wastes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
