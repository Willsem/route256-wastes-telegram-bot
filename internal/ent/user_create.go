...
func (uc *UserCreate) SetWasteLimit(u uint64) *UserCreate {
	uc.mutation.SetWasteLimit(u)
	return uc
}

// SetNillableWasteLimit sets the "waste_limit" field if the given value is not nil.
func (uc *UserCreate) SetNillableWasteLimit(u *uint64) *UserCreate {
	if u != nil {
		uc.SetWasteLimit(*u)
	}
	return uc
...
func (uc *UserCreate) AddWasteIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddWasteIDs(ids...)
	return uc
}

// AddWastes adds the "wastes" edges to the Waste entity.
func (uc *UserCreate) AddWastes(w ...*Waste) *UserCreate {
	ids := make([]uuid.UUID, len(w))
	for i := range w {
		ids[i] = w[i].ID
	}
	return uc.AddWasteIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if value, ok := uc.mutation.WasteLimit(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeUint64,
			Value:  value,
			Column: user.FieldWasteLimit,
		})
		_node.WasteLimit = &value
	}
	if nodes := uc.mutation.WastesIDs(); len(nodes) > 0 {
